% Load barometer data
barometer_data = readtable('/Users/alexmescher/Downloads/Data from phyphox/Pressure.csv'); % Baro output
...
P0 = 102250; % Standard atmospheric pressure at sea level (Pa)
...
% Load GPS data
gps_data = readtable('/Users/alexmescher/Downloads/Data from phyphox/Location.csv'); % Location output
time_gps = gps_data.Time_s_;        % Time from GPS data
altitude_gps = gps_data.Height_m_; % GPS altitude in meters
altitude_gps = altitude_gps - altitude_gps(1);
...
for k = 1:length(time_gps)
    ...
    R_gps = (vertical_uncertainty(k)/10)^2;
